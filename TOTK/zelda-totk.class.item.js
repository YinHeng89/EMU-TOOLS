...
Item.buildHtmlElements=function(item){
	...
	if(item.category==='food'){
		var foodEffects=[
			{name:'无', value:Item.FOOD_EFFECT_NONE},
			{name:'体力恢复', value:Item.FOOD_EFFECT_STAMINA_RECOVER},
			{name:'额外的心心', value:Item.FOOD_EFFECT_LIFE_MAX_EXTRA},
			{name:'额外的精力', value:Item.FOOD_EFFECT_STAMINA_MAX_EXTRA},

			{name:'攻击力提升', value:Item.FOOD_EFFECT_ATTACK_PLUS},
			{name:'低温时冷气攻击', value:Item.FOOD_EFFECT_ATTACK_PLUS_WITH_COLD},
			{name:'高温时火焰攻击', value:Item.FOOD_EFFECT_ATTACK_PLUS_WITH_HEAT},
			{name:'防御提升', value:Item.FOOD_EFFECT_DEFENSE_PLUS},
			{name:'速度提升', value:Item.FOOD_EFFECT_SPEED_PLUS},
			{name:'发光', value:Item.FOOD_EFFECT_BRIGHTNESS},
			{name:'滑落减轻', value:Item.FOOD_EFFECT_SLIP_RESISTANCE},
			{name:'安静性提升', value:Item.FOOD_EFFECT_STEALTH_UP},

			{name:'瘴气伤害恢复', value:Item.FOOD_EFFECT_LIFE_GLOOM_RECOVERY},
			{name:'瘴气防护', value:Item.FOOD_EFFECT_MIASMA_GUARD},
			{name:'火焰防护', value:Item.FOOD_EFFECT_RESIST_BURN},
			{name:'耐热防护', value:Item.FOOD_EFFECT_RESIST_HOT},
			{name:'耐寒防护', value:Item.FOOD_EFFECT_RESIST_COLD},
			{name:'电麻防护', value:Item.FOOD_EFFECT_RESIST_ELECTRIC},
			{name:'游泳速度提升', value:Item.FOOD_EFFECT_SWIM_SPEED_UP}
		];
		...
		item._htmlSelectFoodEffect.title='特效';
		...
		item._htmlInputFoodEffectHearts.title='治愈心心（1/4）';
		...
		item._htmlInputFoodEffectMultiplier.title='等级（1/4）';
		...
		item._htmlInputFoodEffectTime.title='持续时间（秒）';


		item._htmlSpanFoodEffectUnknownValue=span(item.foodEffectUnknown1);
		item._htmlSpanFoodEffectUnknownValue.title='未知值';
	}
}
...
xhr.open('GET', './zh_cn.json', true);
xhr.onload = function () {

	var translations = JSON.parse(this.responseText);

	// 定义分类变量数组
	var categories = Object.keys(Item.TRANSLATIONS);

	// 遍历每个分类变量数组，更新对应的 Item.TRANSLATIONS 对象
	categories.forEach(function (category) {
		var translationsItem = translations[category];
		if (translationsItem) {
			for (var key in Item.TRANSLATIONS[category]) {
				if (translationsItem.hasOwnProperty(key)) { // 如果 translationsItem 分类中包含 Item.TRANSLATIONS 对应的汉化文本，则替换
					Item.TRANSLATIONS[category][key] = translationsItem[key];
				} else { // 如果都没有，则保留英文
					Item.TRANSLATIONS[category][key] = Item.TRANSLATIONS[category][key];
				}
			}
		}
	});
