...
Armor.buildHtmlElements=function(item){
	//build html elements
	var dyeColors=[
		{name:'默认颜色', value:Armor.DYE_NONE},
		...
		{name:'浅蓝色', value:Armor.DYE_LIGHT_BLUE},
		{name:'深蓝色', value:Armor.DYE_NAVY},
		{name:'橙色', value:Armor.DYE_ORANGE},
		{name:'粉色', value:Armor.DYE_PINK},
		{name:'深红色', value:Armor.DYE_CRIMSON},
		{name:'浅黄色', value:Armor.DYE_LIGHT_YELLOW},
		{name:'棕色', value:Armor.DYE_BROWN},
...
xhr.onload = function () {
    ...
    var armors = translations.armors || {};


    for (var key in Armor.TRANSLATIONS) {
        if (armors.hasOwnProperty(key)) { // 如果armors分类中包含Armor.TRANSLATIONS对应的汉化文本，则替换
            Armor.TRANSLATIONS[key] = armors[key];
        } else { // 如果都没有，则保留英文
            Armor.TRANSLATIONS[key] = Armor.TRANSLATIONS[key];
        }
    }
};
