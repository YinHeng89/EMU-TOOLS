...
SavegameEditor={
	...
	_createItemRow:function(item){
		...
		if(typeof item.fuseId==='string' && item.fuseId){
			spanItemId.innerHTML+=' <small style="color:#3d5b50">(合成物: '+item.fuseId+')</small>';
		}
		...
		if(item.category==='materials' || item.category==='food' || item.category==='devices' || item.category==='key'){
			...
			item._htmlDeleteButton.addEventListener('click', function(){
				MarcDialogs.confirm('确定要删除 <strong>'+item.getItemTranslation()+'</strong>?<br/><div style="color:red">警告：使用此功能造成的数据丢失风险自负</div>', function(){
					MarcDialogs.close();
	...
	addKorokPins:function(start, end){
		...
		this.refreshMapPinsCounter();
		MarcDialogs.alert(count+' 个图章已添加');
		return count;
	},
	clearAllMapPins:function(){
		...
		this.refreshMapPinsCounter();
		MarcDialogs.alert(count+' 个图章已删除');
		return count;
	},
