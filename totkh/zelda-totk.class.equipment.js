...
Equipment.buildHtmlElements=function(item){
	...
	item._htmlInputDurability.title='耐久度';
	item._htmlInputDurability.maxValue=item.getMaximumDurability();
	item._htmlInputDurability.value=item.durability;

	//build html elements
	var modifiers=[
		{name:'无强化', value:Equipment.MODIFIER_NO_BONUS},
		{name:'攻击 ↑', value:Equipment.MODIFIER_ATTACK},
		{name:'攻击 ↑↑', value:Equipment.MODIFIER_ATTACK2},
		{name:'耐用 ↑', value:Equipment.MODIFIER_DURABILITY},
		{name:'耐用 ↑↑', value:Equipment.MODIFIER_DURABILITY2}
	];
	if(item.category==='weapons'){
		modifiers.push({name:'暴击↑', value:Equipment.MODIFIER_CRITICAL_HIT});
		...
		//modifiers.push({name:'连发 x3', value:Equipment.MODIFIER_ARROWX3}); //???
		modifiers.push({name:'连发 x5', value:Equipment.MODIFIER_ARROWX5});
	}else if(item.category==='shields'){
		modifiers.push({name:'盾防护 ↑', value:Equipment.MODIFIER_BLOCK});
		modifiers.push({name:'盾防护 ↑↑', value:Equipment.MODIFIER_BLOCK2});
	}
	var unknownModifier=[
	...
	item._htmlSelectModifier.title='强化类型';
	...
	item._htmlInputModifierValue.title='强化值';
...
xhr.onload = function () {
	...
	// 定义分类变量数组
	var categories = Object.keys(Equipment.TRANSLATIONS);

	// 遍历每个分类变量数组，更新对应的 Equipment.TRANSLATIONS 对象
	categories.forEach(function (category) {
		var translationsItem = translations[category];
		if (translationsItem) {
			for (var key in Equipment.TRANSLATIONS[category]) {
				if (translationsItem.hasOwnProperty(key)) { // 如果 translationsItem 分类中包含 Equipment.TRANSLATIONS 对应的汉化文本，则替换
					Equipment.TRANSLATIONS[category][key] = translationsItem[key];
				} else { // 如果都没有，则保留英文
					Equipment.TRANSLATIONS[category][key] = Equipment.TRANSLATIONS[category][key];
				}
			}
		}
	});
